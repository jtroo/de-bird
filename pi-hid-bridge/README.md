...
The deployment scripts use the [Pi Shell](https://github.com/mcyork/pi-shell) tool:

```bash
# Install Pi Shell (pip installable)
...
pi send --pi keybird app/pi_kb.py /home/pi/pi-hid-bridge/app/pi_kb.py
pi run-stream --pi keybird 'sudo systemctl restart pi-hid-bridge.service'

# Web UI changes (HTML/CSS/JS) - No restart needed, just refresh browser
pi send --pi keybird templates/index.html /home/pi/pi-hid-bridge/templates/index.html
pi send --pi keybird static/css/style.css /home/pi/pi-hid-bridge/static/css/style.css
pi send --pi keybird static/js/app.js /home/pi/pi-hid-bridge/static/js/app.js
...
```bash
# With Pi Shell
pi run-stream --pi keybird 'sudo journalctl -u pi-hid-bridge.service -f'
...
- **LEARNING_MODE_GUIDE.md** - How to map unknown keys
- **MOUSE_CONTROL_GUIDE.md** - Web trackpad usage  
- **MOUSE_PASSTHROUGH_GUIDE.md** - Physical mouse forwarding
- **TRACKPAD_CALIBRATION_GUIDE.md** - Calibrate for displays
- **YUBIKEY_SUPPRESS_EXAMPLE.md** - YubiKey Enter suppression
- **KEYBOARD_VENDOR_NOTES.md** - Vendor ID explanations
- **PROFILES_AND_MAPPINGS.md** - Emulation vs mappings
...
For issues, check the main project [README.md](../README.md) troubleshooting section.
